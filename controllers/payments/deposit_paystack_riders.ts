...
exports.callbackPaystack = async (req: Request, res: Response) => {
  ...
  paystack.transaction.verify(reference).then((response: any) => {
    ...
    if (status === 'success') {
      ...
      MySQLConnection.query(SQLCOMMAND, response.data.customer.email, function (err: Error, result: any) {
        if (err) {
          console.error(err);
          return;
        }
...
exports.getBalance = function (req: Request, res: Response) {
  ...
  MySQLConnection.query(query, values, function (err: string, result: any) {
    if (err) {
